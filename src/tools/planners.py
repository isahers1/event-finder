...
def distance_planner(start_location: str, end_location: str):
    ...
    return agent.invoke({"messages": [HumanMessage(content=f"How long is it to drive from {start_location} to {end_location}?")]})['messages'][-1].content

@tool
def route_planner(location: str, distance: int):
    """Find routes in a specific city."""
    agent = create_react_agent(model=model, tools=[search_tool])
    return agent.invoke({"messages": [HumanMessage(content=f"Please find some routes in {location} that are around {distance} miles long")]})['messages'][-1].content
